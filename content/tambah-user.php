                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_level" id="" class="form-control">
                                ...
                                <?php while ($rowlevel = mysqli_fetch_assoc($level)) : ?>
                                    <option <?php isset($rowEdit['id_level']) ? ($rowEdit['id_level'] == $rowlevel['id'])  ? 'selected' : '' : '' ?> value="<?php echo $rowlevel['id'] ?>"><?php echo $rowlevel['nama_level'] ?></option>
                                <?php endwhile ?>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="" class="from-label">Nama Lengkap</label>
                            <input value="<?php echo ($rowEdit['nama_lengkap'] ?? '') ?>" class="form-control" name="nama_lengkap">
                        </div>
                        <div class="mb-3">
                            <label for="" class="from-label">Email</label>
                            <input value="<?php echo ($rowEdit['email'] ?? '') ?> " class="form-control" name="email">
                        </div>
                        <div class="mb-3">
